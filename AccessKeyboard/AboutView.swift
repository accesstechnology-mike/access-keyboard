...
struct AboutView: View {
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                Text("An assistive iPad keyboard that starts from the standard iPadOS layout, especially the large iPad Pro board. Extra tools come later without throwing away the layout you already know. ABC, frequency, handedness, literacy font, coloured vowels, and high-contrast themes are optional; QWERTY stays the starting board.")
                    .foregroundStyle(.secondary)
    ...
    private var privacyText: String {
        var text = "Full Access is requested so layout, colour, contrast, and font settings can be shared with the keyboard extension, and so Fix can call the correction proxy. Keystrokes stay on this device. Tapping Fix sends the current field’s text to that proxy and does not store it there."
