...
A TestFlight row under **Previously Tested** that says “No TestFlight builds are available” and still shows 0.1.0 (1) is a tester without a live invite (`REVOKED` or `NOT_INVITED`), not a missing build. Adding them to Alpha is a no-op. Delete the tester and create a new EMAIL invite. `POST /v1/betaTesters` may include **only one** relationship (`betaGroups` or `builds`, not both). A ghost `REVOKED` row can 404 on DELETE and then `409 Tester(s) cannot be assigned` on Alpha. Treat 404 delete as already gone. Creating on the latest build can return an id while Apple still reports `NOT_INVITED` — trust Apple’s `state`, do not mark `INVITED` locally. App Store Connect **users** on 28 Aug were only `admin@accesstechnology.co.uk` and `mike@accesstechnology.co.uk`. `grace@accesstechnology.co.uk` (GC avatar) is not a team user, so she cannot be an internal Alpha tester until she accepts an App Store Connect user invite (Marketing is the least privileged role Apple documents for internal testers). `mike.thrussell@googlemail.com` is EMAIL `INVITED` without being a user. Live states are only `INVITED`, `ACCEPTED`, `INSTALLED`. Do not treat `409 Tester(s) cannot be assigned` as “already exists”.
